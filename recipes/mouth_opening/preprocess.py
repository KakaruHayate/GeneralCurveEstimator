...
@click.option('--use_lpc', is_flag=False, help='Extract LPC coefficients.')
@click.option('--lpc_order', default=20, type=int, help='LPC order required for extracting LPC coefficients')
...
def preprocess(
    ...
    if use_lpc:
        from lib.lpctorch import LPCCoefficients
    ...
    with tqdm.tqdm(csv_list) as bar:
        for csv_file in bar:
            ...
            for audio_file in csv_file.parent.glob("*.wav"):
                ...
                if use_lpc:
                    lpc = lpc_prep(torch.from_numpy(audio).unsqueeze(0)).squeeze(0).detach().numpy()
                    lpc = numpy.pad(lpc, ((0, mel.shape[0] - lpc.shape[0]), (0, 0)), mode='constant') # pad zero
                else:
                    lpc = None
                if use_vad:
                ...
                if lpc is not None:
                    lpc = numpy.delete(lpc, frame_indices, axis=0)
                # save npz
                target_file = target_dir / audio_file.relative_to(source_dir).with_suffix(".npz")
                target_file.parent.mkdir(parents=True, exist_ok=True)
                numpy.savez(target_file, spectrogram=mel, curve=curve, lpc=lpc)
