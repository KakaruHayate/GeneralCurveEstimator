...
class BiLSTMCurveEstimator(nn.Module):
    """
    BiLSTM-based Mel-spectrogram & LPC Coefficients Prediction Encoderc in Curve Estimation

    Args:
        in_dims (int): Number of input channels, should be same as the number of bins of mel-spectrogram & LPC Coefficients.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x (torch.Tensor): Input mel-spectrogram & LPC Coefficients, shape (B, T, input_channels) or (B, T, mel_bins+LPC_order).
        return:
